subroutine send_data_mpi_dcp( i_proc, n_elem, send_data, tag, b_verbose )
    ...
    double complex:: send_data(n_elem);
    integer:: mpi_ierr;
    character(len=200):: text;

    call mpi_send( send_data, n_elem, MPI_DOUBLE_COMPLEX, &
...
end subroutine send_data_mpi_dcp



subroutine recv_data_mpi_dcp( i_proc, n_elem, recv_data, tag, b_verbose )
    ...
    integer, intent(in):: i_proc, n_elem, tag, b_verbose;
    double complex:: recv_data(n_elem);
    integer:: mpi_ierr, status(4);
    character(len=200):: text;

    call mpi_recv( recv_data, n_elem, MPI_DOUBLE_COMPLEX, i_proc, &
...
end subroutine recv_data_mpi_dcp
...
subroutine transfer_mpi_data_dcp( n_proc, i_proc, n_assign, data, mpi_data, flag )
    ...
    if( associated( mpi_data ) ) then
        ...
        stop "transfer_mpi_data_dcp: array not yet allocated!";
    end if
