...
module mod_sub_bkw_prop
    implicit none
    double precision:: m_t0, m_ti
end module mod_sub_bkw_prop

subroutine sub_bkw_prop( ts, ierr, z_t0, x_t0, vz_t0, vx_t0, w, err_spe, tag )
    ...
    double complex, external:: action_w_sub_bkw
    integer:: b_last
    ...
    integer:: i
    external:: newton_equation_sub_bkw

    m_t0 = dreal(ts)
    m_ti = dimag(ts)

    h0 = (m_ti - 0d0) / (nt - 1d0)
    y0(1) = sub_traj_x_0( dcmplx(m_t0, 0d0), ts )
    y0(2) = eye*sub_traj_vx_0( dcmplx(m_t0, 0d0) )
    y0(3) = sub_traj_z_0( dcmplx(m_t0, 0d0), ts )
    y0(4) = eye*sub_traj_vz_0( dcmplx(m_t0, 0d0) )
...
20  b_last = 0
    ...
    t = t_old
    y = y_old
    call rk4_sub( h, t, y, y_mid, newton_equation_sub_bkw )
    t = t + h
    y = y_mid
    call rk4_sub( h, t, y, y2, newton_equation_sub_bkw )
...
30    ierr = 0
    ...
    call rk4_plot_open_file()
    do i = 1, n_step+1
        y_actual(1) = array_x(i)
        y_actual(2) = array_vx(i)
        y_actual(3) = array_z(i)
        y_actual(4) = array_vz(i)
    ...
    return    
end subroutine sub_bkw_prop

! --------------------------------------------------------------------------------
subroutine newton_equation_sub_bkw( ne, t, y, dy )
    use mod_sub_bkw_prop, only: m_t0,  m_ti
    implicit none
    ...
    return
end subroutine newton_equation_sub_bkw
